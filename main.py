...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    deconv_layer_1 = tf.layers.conv2d_transpose(
        inputs=conv_layer_7,
        filters=num_classes,
        kernel_size=4,
        strides=2,
    ...
    skip_connection_1 = tf.add(deconv_layer_1, conv_layer_4)
    ...
    skip_connection_2 = tf.add(deconv_layer_2, conv_layer_3)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            # Training
            loss, _ = sess.run([cross_entropy_loss, train_op], feed_dict={input_image: image,
                                                                          correct_label: label,
                                                                          keep_prob: 0.8,
                                                                          learning_rate: 1e-4})
...
def run():
    ...
    with tf.Session() as sess:
        ...
        sess.run(tf.global_variables_initializer()) # TODO: Move?
